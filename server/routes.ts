...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/user-profile/:userId/:serverId", async (req, res) => {
    try {
      ...
      // 레벨 진행도 계산 (올바른 공식)
      const currentLevelXP = userServer.level === 1 ? 0 : calculateRequiredXP(userServer.level - 1);
      const nextLevelXP = calculateRequiredXP(userServer.level + 1);
